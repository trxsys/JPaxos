...
public abstract class SimplifiedService extends AbstractService {
     ...
     * @return generated reply which will be sent to client
     */
    protected abstract byte[] execute(byte[] value, int batchOrderId);
    ...
    public final byte[] execute(byte[] value, int seqNo, int batchOrderId) {
        lastExecutedSeq = seqNo;
        return batchOrderId == -1 ? execute(value) : execute(value, batchOrderId);
    }
