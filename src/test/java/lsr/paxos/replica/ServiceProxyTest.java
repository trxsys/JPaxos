...
public class ServiceProxyTest {
    ...
    public void shouldExecuteSingleRequestOnService() {
        ...
        serviceProxy.execute(request, batchOrderId);

        verify(service).execute(new byte[] {1, 2, 3}, nextSeqNo - 1, 0);
    }
    ...
    public void shouldExecuteRequestsOnService() {
        ...
        serviceProxy.execute(request1, batchOrderId);
        serviceProxy.instanceExecuted(0);
        serviceProxy.execute(request2, batchOrderId);
        serviceProxy.instanceExecuted(1);

        verify(service).execute(new byte[] {1}, nextSeqNo - 1, 0);
        verify(service).execute(new byte[] {2}, nextSeqNo - 1, 1);
    }
    ...
    public void shouldSkipRequestsAfterUpdatingToSnapshot() {
        ...
        serviceProxy.updateToSnapshot(snapshot);
        verify(service).updateToSnapshot(7, snapshot.getValue());
        byte[] skipped1 = serviceProxy.execute(RequestGenerator.generate(), batchOrderId);
        byte[] skipped2 = serviceProxy.execute(RequestGenerator.generate(), batchOrderId);

        assertArrayEquals(reply1.getValue(), skipped1);
        assertArrayEquals(reply2.getValue(), skipped2);
        verify(service, never()).execute(any(byte[].class), nextSeqNo - 1, anyInt());

        Request firstExecuted = RequestGenerator.generate();
        serviceProxy.execute(firstExecuted, batchOrderId);
        verify(service).execute(firstExecuted.getValue(), nextSeqNo - 1, 7);
    }
    ...
    public void shouldHandleSnapshotFromService() {
        serviceProxy.execute(RequestGenerator.generate(), batchOrderId);
        serviceProxy.execute(RequestGenerator.generate(), batchOrderId);
        serviceProxy.execute(RequestGenerator.generate(), batchOrderId);
        serviceProxy.instanceExecuted(0);
        serviceProxy.execute(RequestGenerator.generate(), batchOrderId);
    ...
    public void shouldHandleSnapshotFromServiceCalledWithinExecuteMethod() {
        ...
        serviceProxy.execute(lastRequest, batchOrderId);
